...
- [x] IS-LM にゼロ金利制約（流動性の罠）を追加（ZLB=0%、金利クランプ+UIP調整、zlb_binding フラグ表示）（PR #33）
- [x] ベースライン金利・為替を FRED/Yahoo 実データから動的取得（JGB 10Y=2.35%, USD/JPY=159.3円）（PR #33）
- [x] UIP 感応度をUIスライダーで調整可能に（0〜10円/pp、IS-LMエンジン時のみ表示）（PR #33）

### P4: UX改善
- [x] 資金需要チャートに系列の表示/非表示トグル、ネット資金需要の集約表示を追加（PR #32）
